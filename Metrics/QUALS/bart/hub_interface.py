...
class BARTHubInterface(nn.Module):
    ...
    def __init__(self, args, task, model):
        ...
        # self.bpe = register_bpe(dictionary=Dictionary.load('path/to/dict.txt'))
    ...
    def encode(self, sentence: str, *addl_sentences, no_separator=True, no_bos=False,
               input_is_bpe=False) -> torch.LongTensor:
        """
    ...
    def generate(self, tokens: List[torch.LongTensor], beam: int = 5, verbose: bool = False, return_all: bool = False, **kwargs) -> torch.LongTensor:
        ...
        if return_all and beam > 1:
            hypos = [x[:beam] for x in translations]
